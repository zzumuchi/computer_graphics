    ...
    function createDoubleWall(name, x, y, z, width, height, depth) {
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                const mesh = new THREE.Mesh(brickGeo, wallMat.clone());
                const hPos = startH + c;
                const vPos = startV + r;
...
export function createFixedObstacle(x, y, z, w = 1, h = 1, d = 1, color = 0x444444) {
    ...
    const mat = new THREE.MeshStandardMaterial({ color: color, roughness: 0.9, metalness: 0.1, transparent: true, opacity: 1.0 });
...
export function createLaserSource(x, y, z, dir) {
    ...
    group.add(body);
    const lens = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.6, 0.1), new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0x000000 }));
...
export function createLaserSensor(x, y, z, dir, targetColor = 0xffffff) {
    ...
    const body = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1), new THREE.MeshStandardMaterial({ color: 0x222222 }));
    group.add(body);
    const lens = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.7, 0.05), new THREE.MeshStandardMaterial({ color: 0x004400, emissive: 0x000000 }));
    const dirKey = dir.join(','); 
    switch (dirKey) {
        case "1,0,0":  lens.position.x = 0.5; lens.rotation.y = Math.PI/2; break;
        case "-1,0,0": lens.position.x = -0.5; lens.rotation.y = -Math.PI/2; break;
...
export function createMirrorCube(x, y, z) {
    ...
    geometry.setAttribute('position', new THREE.BufferAttribute(vertices, 3)); geometry.computeVertexNormals(); geometry.addGroup(0, 6, 1); geometry.addGroup(6, 18, 0); 
    const structMat = new THREE.MeshStandardMaterial({ color: 0x333333, roughness: 0.8, metalness: 0.2, side: THREE.DoubleSide });
    const mirrorMat = new THREE.MeshStandardMaterial({ color: 0xaaccff, roughness: 0.05, metalness: 1.0, emissive: 0x112233, emissiveIntensity: 0.2, side: THREE.DoubleSide });
    const prismMesh = new THREE.Mesh(geometry, [structMat, mirrorMat]); 
    prismMesh.userData = { isPrism: true }; prismMesh.castShadow = true; prismMesh.receiveShadow = true; group.add(prismMesh);
...
export function createTrapezoidMirrorCube(x, y, z) {
    ...
    const structMat = new THREE.MeshStandardMaterial({ color: 0x333333, roughness: 0.7 });
    const mirrorMat = new THREE.MeshStandardMaterial({ color: 0x00ffff, roughness: 0.05, metalness: 1.0, emissive: 0x002222, emissiveIntensity: 0.2 });
...
export function createHalfMirrorCube(x, y, z) {
    ...
    const mirrorMat = new THREE.MeshStandardMaterial({ color: 0x00ff00, roughness: 0.0, metalness: 1.0, emissive: 0x222222, emissiveIntensity: 0.2 });
    const mesh = new THREE.Mesh(geometry, [structMat, mirrorMat]);
    mesh.userData = { isPrism: true }; mesh.castShadow = true; mesh.receiveShadow = true; group.add(mesh);
...
export function createDispersionCube(x, y, z) {
    ...
    const edges = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(1, 1, 1)), new THREE.LineBasicMaterial({ color: 0x00ffff }));
    edges.userData = { ignoreLaser: true }; group.add(edges);
    ...
    outline.name = 'selectionOutline'; outline.userData = { ignoreLaser: true }; group.add(outline);
